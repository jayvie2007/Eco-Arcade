...
// Setting up for switch button
int is_switch_press = 0; 
int debounce_delay = 300;
int switch_button = D7;


// Setting up for DHT/Temperature,Humidity Sensor
...
// pin SDA = D1 SCL = D2, Setup for LCD
...
void loop() {
    // Reading the Switch press status
    is_switch_press = digitalRead(switch_button); 

    // Get data from firebase for count of bottles
    ...
    // Function for switch if pressed
    if (is_switch_press == LOW){
        ...
        delay(800);
         ...
         // Function for printer if conditions met
    ...
    // Function for switch if not pressed
    } else{
